...
if (isset($_SESSION['id']))
{
    ...
    if (isset($_POST['newname']) AND !empty($_POST['newname']) AND $_POST['newname'] != $user['nom'])   # Nouveau nom
    {
    ...
    if (isset($_POST['newprenom']) AND !empty($_POST['newprenom']) AND $_POST['newprenom'] != $user['prenom'])   # Nouveau prénom
    {
    ...
    if (isset($_POST['newformation']) AND !empty($_POST['newformation']) AND $_POST['newformation'] != $user['formation']) # Nouvelle formation
    {
        $newformation = htmlspecialchars($_POST['newformation']);
        $insertformation = $bdd->prepare("UPDATE membres SET formation = ? WHERE id = ?");
        $insertformation->execute(array($newformation, $_SESSION['id']));
       header('Location: profil.php?id='.$_SESSION['id']);
    }
    if (isset($_POST['newadresse']) AND !empty($_POST['newadresse']) AND $_POST['newadresse'] != $user['adresse']) # Nouvelle adresse
    {
    ...
    if (isset($_POST['newcp']) AND !empty($_POST['newcp']) AND $_POST['newcp'] != $user['code_postal']) # Nouveau code postal
    {
        $newcp = htmlspecialchars($_POST['newcp']);
        $insertcode_postal = $bdd->prepare("UPDATE membres SET code_postal = ? WHERE id = ?");
        $insertcode_postal->execute(array($newcp, $_SESSION['id']));
       header('Location: profil.php?id='.$_SESSION['id']);
    }


    if (isset($_POST['newmail']) AND !empty($_POST['newmail']) AND $_POST['newmail'] != $user['mail']) # Nouveau mail
    {
    ...
    if (isset($_POST['newmdp1']) AND !empty($_POST['newmdp1']) AND isset($_POST['newmdp2']) AND !empty($_POST['newmdp2'])) 
    {
        ...
        $mdp2 = sha1($_POST['newmdp2']);    # Nouveau mot de passe
    ...
    if (isset($_FILES['avatar']) AND !empty($_FILES['avatar']['name']))   # Nouvelle photo de profil
    {
        $tailleMax = 2097152;
  ...
  <head>
    ...
    <br /> 
        <div align="center">
          ...
          <a class="btn btn-primary btn-xl" href="profil.php?id=<?= $_SESSION['id'] ?>">Retourner sur mon profil</a>        <br/>  <br/>
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                  ...
                  <div class="control-group">
                    <label>Nom:</label>
                    <input class="form-control" type="text" name="newname" placeholder="Nom" value="<?php echo $user['nom'];?>"/><br/><br/>
                  <label>Prénom:</label>
                  <input class="form-control" type="text" name="newprenom" placeholder="Prénom" value="<?php echo $user['prenom'];?>"/><br/><br/>
                  <label>Mail:</label>
                  <input class="form-control" type="text" name="newmail" placeholder="Mail" value="<?php echo $user['mail'];?>"/><br/><br/>
                   <label>Formation:</label>
                  <input class="form-control" type="text" name="newformation" placeholder="Formation" value="<?php echo $user['formation'];?>"/><br/><br/>
                  <label>Adresse:</label>
                  <input class="form-control" type="text" name="newadresse" placeholder="Adresse" value="<?php echo $user['adresse'];?>"/><br/><br/>
                  <label>Code postal:</label>
                  <input class="form-control" type="text" name="newcp" placeholder="Code postal" value="<?php echo $user['code_postal'];?>"/><br/><br/>
                  <label>Mot de passe:</label>
                  <input class="form-control" type="password" name="newmdp1" placeholder="Mot de passe"/><br/><br/>
                  <label>Confirmation mot de passe:</label>
                  <input class="form-control" type="password" name="newmdp2" placeholder="Confirmation mot de passe"/><br/><br/>
                  <label>Avatar:</label>
                  <input class="form-control" type="file" name="avatar"/><br/><br/>
                  <input class="btn btn-primary btn-xl" type="submit" value="Mettre à jour mon profil" name="envoyer"/><br/><br/>
                </div>
            </form>
